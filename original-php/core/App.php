...
class App
{
    ...
    public function route()
    {
        ...
        if ($url) {
            ...
            $total_count = count($urlArray);
            // 获取控制器名
            $controllerName = ucfirst($urlArray[$total_count-2]);
            ...
            $actionName = $urlArray[$total_count-1];
        ...
        $dispatch = new $controller($controllerName, $actionName,$this->config);

        //2 执行base类文件的方法 预先判断是否登陆过 如果过不需要登陆的接口验证 不需要集成base文件
        if(method_exists($dispatch,'beforeAction')){
            if(!$dispatch->beforeAction()) return;
        }

        //3 如果存在初始化方法 init 先执行初始化方法
    ...
    public function setDbConfig()
    {
        if ($this->config['db']) {
            //注册到全局
            $GLOBALS['db_conf'] = $this->config['db'];
